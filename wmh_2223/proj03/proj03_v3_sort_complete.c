...
void passwd(void);
char password[1000]="$";
int changePasswordFlag=0,goodsIndex=0;
typedef struct{
    char name[32];
    int price ;
    int quantity;
...
int main(){
    ...
    /*strcpy(goods[0].name,"b");
    goods[0].quantity=18;
    strcpy(goods[1].name,"a");
    goods[1].quantity=53;    
...
int sellerMode(){
    ...
    int quotationFlag,argCounter,j,index;
    
    /*if(!changePasswordFlag){uncomment
        //passwd();
    }
    ...
    while(1){
        ...
        if(argCounter>3){//stag(start argument)
            ...
            sort(arg[2]);
            for(int i=0;i<goodsIndex;i++){
                printf("%d %s %d %d\n",i,goods[i].name,goods[i].price,goods[i].quantity);
            }
        }
...
void sort(char* order){
    if(strcmp(order,"name")==0){
        ...
        for(int i=0;i<goodsIndex;i++){
            for(int j=0;j<goodsIndex-i-1;j++){
                if(goods[j].quantity>goods[j+1].quantity){
                    goodsTemp=goods[j];
                    goods[j]=goods[j+1];
                    goods[j+1]=goodsTemp;
                }
