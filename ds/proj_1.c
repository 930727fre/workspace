...
void bfs(int (*src), int (*src2), int (*dst), int maze[1000][1000]){
    ...
    push_back(src[1], src[0], &front, &last, NULL, maze);
    current=front;
    while(!empty(current)){
        if(current->x==dst[1]&&current->y==dst[0]){ //if arrive the destination
...
int main(){
    ...
    bfs(src[0], src[1], dst[0], maze);
    bfs(src[1], nullSrc, dst[1], maze);
    // clear(front);
